...
CONF = cfg.CONF
CONF.import_opt('heartbeat_timeout', 'ironic.conductor.manager', group='conductor')
...
class Connection(api.Connection):
    ...
    def _add_nodes_filters(self, query, filters):
        ...
        if 'provisioned_before' in filters:
            limit = timeutils.utcnow() - datetime.timedelta(seconds=filters['provisioned_before'])
            query = query.filter(models.Node.provision_updated_at < limit)
        if 'inspection_started_before' in filters:
            limit = ((timeutils.utcnow()) - (datetime.timedelta(seconds=filters['inspection_started_before'])))
    ...
    def get_xclarity_by_ipaddress(self, xclarity_ipaddress):
        query = model_query(models.xClarityInstance).filter_by(ipaddress=xclarity_ipaddress)
        try:
            return query.one()
        except NoResultFound:
    ...
    def create_xclarity(self, values):
        ...
        try:
            ...
            raise exception.XClarityAlreadyExists(ipaddress=values['ipaddress'])
        return xclarity
    ...
    def create_pod_manager(self, values):
        pod_manager = models.DriverInstanceServer()
    ...
    def get_rsa_chassis_list_by_pod_and_type(self, pod_id, chassis_type, limit=None, marker=None, sort_key=None,
        ...
        # query = model_query(models.RSAChassis).filter(pod_id=pod_id, RSAChassis.chassis_type!='Rack')
    ...
    def destroy_computer_system(self, pod_id):
        ...
        try:
            with session.begin():
                ...
                # delete interfaces
                interfaces_query = model_query(models.Interface).filter(models.Interface.computer_system_id.in_(system_ids))
    ...
    def get_target_list(self, pod_id, limit=None, marker=None, sort_key=None, sort_dir=None):
    ...
    def get_pcieswitch_list(self, pod_id, limit=None, marker=None, sort_key=None, sort_dir=None):
